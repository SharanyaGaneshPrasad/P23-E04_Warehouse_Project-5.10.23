...
def placing_order(search_item,item_count_in_warehouse):
    total_item_count_in_Warehouses= sum([i[1] for i in item_count_in_warehouse])#sum(item_count_in_warehouse_dict.values())
    print(f"{item_count_in_warehouse}")
    ...
    if order_quantity <= total_item_count_in_Warehouses:
        ...
        print(f"{"%"*150}")
        for warehouse_count in item_count_in_warehouse:    
            for warehouse in stock:
                if str(warehouse)==warehouse_count[0]:
                    print(f"warehouse: {warehouse}, warehouse_count: {warehouse_count}")
                    for item in warehouse.stock:
                        if str(item).lower()==search_item.lower()  and order_quantity>0:
        ...
        for warehouse_count in item_count_in_warehouse:    
            for warehouse in stock:
                if str(warehouse)==warehouse_count[0]:
                    print(f"warehouse: {warehouse}, warehouse_count: {warehouse_count}")
                    for item in warehouse.stock:
                        if str(item).lower()==search_item.lower()  and total_item_count_in_Warehouses>0:
                            print(f"order quantity: {total_item_count_in_Warehouses}")
                            warehouse.stock.remove(item)
                            total_item_count_in_Warehouses-=1
...
def search_n_order(actions,authorized_employee):
    location, item_count_in_warehouse_dict, search_item = search_item_name()
    if len(location)>0:
        ...
        if isinstance(authorized_employee, Employee)==True:
            ...
            if place_order.lower() in ("y", "Y"):
                item_count_in_warehouse = sorted(item_count_in_warehouse_dict.items(), key=lambda x: x[1], reverse=True) #sort warehouse based on item counts
                placing_order(search_item,item_count_in_warehouse)
    else:
        print(f"{colors.ANSI_RED}\nNot in stock")

    actions.append(f"Searched for {search_item}.   {"-"*10} {datetime.now()}")
...
def category_selection(actions, authorized_employee):
    ...
    if not category_name==None:
        ...
        print("."*120)
        actions.append(f"Browsed the category {category_name}.  {"-"*10} {datetime.now()}")
    continue_session = input(f"\n{":"*20}  {colors.ANSI_PURPLE}Do you want to continue with another operation? (y/n){colors.ANSI_RESET}  {":"*20}   -   {colors.ANSI_YELLOW}")
    if continue_session in ("y", "Y"):
        run(actions, authorized_employee)
...
def run(actions, authorized_employee=None):
    ...
    if menu_selection == "1":
        ...
        actions.append(f"listed {total_items} items from {len(new_item_dict.keys())} Warehouses.  {"-"*10} {datetime.now()}")
    # actions.append("Hi")
        continue_session = input(f"\n{":"*20}  {colors.ANSI_PURPLE}Do you want to continue with another operation? (y/n){colors.ANSI_RESET}  {":"*20}   -   {colors.ANSI_YELLOW}")
...
def start_shopping():
    ...
    if isinstance(authorised_employee, Employee):
        authorised_employee.bye(actions)
        user_log_path=os.path.join(BASE_DIR, "log/employee_log.txt")
        with open(user_log_path, 'a') as file1:
            file1.write(f"\nEmployee : {username}\n")
            file1.writelines(actions)
